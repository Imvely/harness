...
내부 도구에 입히는 것도 문제지만, 검증도 되어 있지 않았다 — `#fbbc04`는 lightness band에서
탈락(0.83)하고 대비 1.66:1인데 SVG `fill`로 실제 쓰이고 있었다.

지금 값은 dataviz 스킬의 검증된 레퍼런스 팔레트에서 가져왔고, 세 역할을 분리해서 쓴다.

| 토큰 | 용도 | 규칙 |
|---|---|---|
| `--series-*` | 차트·그래프의 **마크만** | 텍스트에 쓰지 않는다. 범주 집합으로 검증 |
| `--status-*` | 상태의 채움/선 | 항상 아이콘 + 단어와 함께 나온다 |
| `--success` / `--warning` / `--danger` | 위 상태의 **잉크(글자)** | 자기 soft 배경 위에서 4.5:1 이상 |

**범주 색은 4개가 한계다.** 레퍼런스 램프의 모든 부분집합을 validator로 전수 검사한 결과,
light/dark 양쪽에서 all-pairs 색각·정상시력 하한을 통과하는 최대 집합이 4색이었다. 그래서
문헌 그래프의 노드 종류 7개 중 4개만 색을 갖고 나머지는 중립색을 쓴다 — 어차피 모든 노드는
원 안에 종류 글자를 찍고 범례가 이를 반복하므로, 색은 보조 수단이다.
(이전에는 `paper`와 `experiment`가 `#1a73e8`/`#4285f4`, 즉 **구분 불가능한 두 파랑**이었다.)

글꼴은 시스템 sans다. `"Google Sans"`는 20개 규칙에 적혀 있었지만 `<link>`도 `@font-face`도
없어서 **전부 Arial로 폴백**되고 있었다. 한국어 UI이므로 폴백 체인에 한글 폰트를 넣었고,
웹폰트는 쓰지 않는다 — 로컬 정적 빌드로 도는 도구에 CDN 요청을 붙일 이유가 없다.

**다크 모드**는 두 번째 값 집합이지 첫 번째를 뒤집은 것이 아니다. 계열 색은 레퍼런스 팔레트의
dark step을 이 표면(`#1a1a19`)에 대해 다시 검증한 것이고, 상태 마크는 설계상 모드 불변이지만
그 **잉크 버전은 아니다** — 글자는 자기가 놓인 표면 위에서 읽혀야 하므로 다시 고르고 WCAG로
다시 쟀다. 선언은 두 번 한다: 미디어 쿼리는 OS를 따르고 `[data-theme]` 속성은 사용자의 선택을
따르며, `:not([data-theme="light"])` 가드가 "OS는 어둡지만 나는 밝게"를 가능하게 한다.
사이드바의 `화면 모드`에서 밝게/어둡게/시스템을 고른다. 첫 페인트 전에 `index.html`의 인라인
스크립트가 저장된 선택을 찍으므로 깜빡임이 없다.

검증: 7개 화면 × 2개 모드의 **모든 텍스트 노드**에 대해 실제 렌더된 전경·배경을 읽어 WCAG
대비를 계산했고 미달 0건이다(스크립트는 세션 스크래치패드). `tests/test_web_mockup_files.py`의
`test_every_colour_in_the_stylesheet_is_a_token`이 토큰 블록 밖의 리터럴 색을 막는다 —
하드코딩된 `#ffffff` 하나가 다크 모드에서 흰 바탕으로 렌더되는데 빌드도 타입 검사도 그걸
잡지 못하기 때문이다.

내보내기를 **기다리는 동안**에는 배너가 출처를 단정하지 않는다. 그 순간 화면의 행은 번들된
예시이고 곧 교체될 수 있으므로, "합성 예시 데이터"라고 말하면 실측값이 될 숫자에 잘못된
라벨을 붙이는 셈이다. 대신 "데이터 출처를 확인하는 중"이라고 말하고 `aria-busy`를 세우며
본문을 흐린다 — 스켈레톤으로 갈아치우지 않는 이유는 독자가 이미 읽을 수 있는 것을 버리기
때문이다.

지표 추세 차트는 **APCER와 AUC를 두 패널로 나눴다**. 한 축에 겹쳐 그리면 "위쪽 = 좋음"이
APCER에서는 틀리고 AUC에서는 맞는, 서로 모순되는 규칙 두 개를 독자가 동시에 들고 있어야 한다.
패널당 계열이 하나이므로 범례도 필요 없다.
