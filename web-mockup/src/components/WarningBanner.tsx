...
/**
 * The standing "what am I actually looking at" line.
 *
 ...
 *
 * It states the data's origin first, because every number below it means something different
 * depending on that origin. It is one line now: the origin in a few words, always visible, and
 * the full explanation and the always-on rules behind the ?.
 */
...
/** `label` is the few words always on screen; `title` and `body` are the full statement. */
type Copy = { tone: string; label: string; title: string; body: string };
...
function copyFor(origin: DataOrigin, locale: Locale): Copy {
  ...
  if (origin === "export_synthetic") {
    return locale === "ko"
      ...
      : {
          tone: "safety-banner--mixed",
          label: "Synthetic-data runs · not a result",
  ...
  if (origin === "mixed") {
    return locale === "ko"
      ? {
          ...
          body: "모의 행은 측정값이 아니라 이 화면이 공식으로 계산해 만든 숫자입니다. 표에서 '모의 값' 표시가 붙은 행은 어떤 비교에도 쓰지 마세요.",
        }
      : {
          tone: "safety-banner--mixed",
          label: "Real + mock rows mixed",
  ...
  return locale === "ko"
    ...
    : {
        tone: "safety-banner--demo",
        label: "Sample data · not a result",
...
export function WarningBanner({
  ...
  const ko = locale === "ko";
  const copy: Copy = resolving
    ? {
        tone: "safety-banner--resolving",
        label: ko ? "데이터 출처 확인 중" : "Checking data origin",
        title: ko ? "데이터 출처를 확인하는 중입니다" : "Checking where these numbers came from",
        body: ko
          ? "지금 보이는 행은 번들된 예시입니다. 내보낸 결과가 있으면 곧 교체되고 이 줄이 출처를 알려줍니다."
          : "The rows below are the bundled samples. If an export is present they will be replaced in a moment, and this line will say so.",
      }
    : copyFor(origin, locale);
  const rules = [
    ko ? "임계값은 dev에서만 (dev threshold only)" : "dev threshold only",
    ko ? "protocol hash 일치 시에만 비교" : "protocol hash checked",
    ko ? "전체 실행은 사람 승인 (human full-run approval)" : "human full-run approval",
  ...
  return (
    <section className={`safety-banner ${copy.tone}`} aria-label={ko ? "표시 중인 데이터의 출처" : "Origin of the data on screen"}>
      <span aria-hidden="true" className="safety-banner__dot" />
      <strong className="safety-banner__label">{copy.label}</strong>
      <Hint align="start" label={ko ? "데이터 출처 설명" : "About the data origin"}>
        <strong className="hint__title">{copy.title}</strong>
        <span>{copy.body}</span>
        <span className="hint__list" aria-label={ko ? "항상 적용되는 규칙" : "Always-on rules"}>
          {rules.map((rule) => (
