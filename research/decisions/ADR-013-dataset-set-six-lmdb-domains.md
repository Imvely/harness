...
- **aihub114는 성능 근거가 될 수 없다**(위 §실측 결과 2). 원본에 GoPro bona-fide가 없으므로 세 선택지 중 "같은 카메라만 쓰기"와 "GoPro bona-fide만 쓰기"는 불가능하다. 남는 것은 **도메인 제외**이고, 빌더의 선택 규칙("bona-fide와 공격을 모두 찍은 카메라만 사용")이 이 도메인을 자동으로 비운다. 사용자 최종 확인이 필요하다.
