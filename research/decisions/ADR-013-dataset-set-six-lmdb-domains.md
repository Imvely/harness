...
## Status
Proposed — 데이터셋 구성은 사용자가 확인했다(2026-09-22, "LMDB에 있는 게 진짜 데이터셋이야. 그거만 있어"). 아래 표는 2026-09-22 서버 실측(`scripts/inspect_lmdb_layout.py`, 6개 도메인 전체 스캔) 결과다. 실측에서 **차단급 문제 두 건**이 나왔고(§실측 결과), 그중 aihub114 제외는 사용자가 확정했다(2026-09-23) — 연구 도메인은 **5개**다. Idiap 중복 키 처리까지 반영되면 Accepted로 바꾼다.
...
**원본 폴더를 확인한 결과, aihub114의 교란은 데이터셋 자체의 성질이다**(2026-09-23 실측). 50명 표본에서 GoPro는 공격 4종만 찍었고(클래스당 150 clip), bona-fide는 전부 스마트폰·태블릿(Galaxy Note20, Z Flip, iPad Pro, Galaxy Tab 등)에 있다. 200명까지 훑어도 **GoPro로 찍은 bona-fide는 하나도 없다.** 조명은 3가지가 모두 있지만 카메라와 라벨의 대응은 조명과 무관하게 고정이다. 즉 **담는 범위를 어떻게 넓혀도 같은 카메라로 찍힌 진짜와 공격을 만들 수 없다.**

**aihub115는 반대로 깨끗하다.** SR305 한 대가 bona-fide와 print 공격 6종을 모두 찍었다(각 50 clip). 카메라로 라벨을 맞힐 수 없다. 덧붙여 **원본에는 공격이 6종인데 팀원 저장소에는 4종만 담겨 있다**(`attack_03_print_eye_nose_flat`, `attack_04_print_eye_nose_curved` 누락). 우리 빌드는 6종을 모두 담는다.
...
- **aihub114는 제외했다**(사용자 확정 2026-09-23). 원본에 GoPro bona-fide가 없어 "같은 카메라만 쓰기"도 "GoPro bona-fide만 쓰기"도 불가능하고, 남는 선택지는 도메인 제외뿐이었다. 연구에 쓰는 도메인은 **5개**다. 빌더의 선택 규칙("bona-fide와 공격을 모두 찍은 카메라만 사용", `data/build_store.py::plan_build`)이 이름 검사 없이 이 도메인을 비우므로, 같은 성질을 가진 도메인이 앞으로 들어와도 같은 방식으로 걸린다.
