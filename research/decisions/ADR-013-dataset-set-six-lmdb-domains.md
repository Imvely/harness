...
사용자가 번호를 확인해 주었다(2026-09-23): `aihub115` → AI Hub **161번**, `aihub114` → AI Hub **83번**.
...
**83번은 맞지 않는다.** 페이지는 "한국인 안면 이미지"(2017년 구축), 대상자 1,000명, 인당 약 3만 장, 총 2,860만 장, jpg, "3개 해상도"라고만 적고 **공격 유형을 명시하지 않는다.** 그런데 저장소의 `aihub114`는 공격 5종(print 2·replay 2·3D mask)에 대상자 1,686명이고 카메라는 GoPro와 스마트폰이다. 둘은 다른 데이터셋으로 보인다.

확정 전까지 `aihub114`의 출처는 **미확인**으로 둔다. 원본 폴더의 설명 파일이나 라벨 JSON으로 대조해 정한다. 페이지 값은 WebFetch 요약을 거친 것이라 브라우저에서 한 번 더 확인해야 한다(같은 사이트에서 요약이 값을 지어낸 전례가 있다).
